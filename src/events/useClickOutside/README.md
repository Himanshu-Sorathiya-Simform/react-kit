...
- **Conditional Rendering & Refs (Late-Mount Gotcha):** If an entry in `target` is a ref whose element renders conditionally after mount, the hook won't detect it — a plain `useRef` mutation doesn't trigger a re-render, so it never sees the ref populate. Use a callback ref or state-based ref instead. See [`useEventListener`'s Late-Mount Gotcha](../useEventListener/README.md#gotchas--edge-cases) for the full explanation.

- **A Click Target That Unmounts Itself Is Treated as "Not Outside":** If the clicked element is no longer connected to the document by the time the handler runs — for example, its own click handler removed it from the DOM synchronously — there's no reliable way to resolve it against `.contains()` for any configured target. Rather than risk a false-positive "outside" firing on a node that may well have been inside, the handler is skipped entirely for that event. This is a narrow edge case in practice: the default capture-phase listener (see below) runs *before* the click's own handlers, so it only matters if you've opted into `capture: false` or the removal happens through some other synchronous path.
