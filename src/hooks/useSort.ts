...
function useSort<T>(data: T[], initialSorts: SortState = []): UseSortReturn<T> {
	...
	const sortedItems = data.toSorted((itemA, itemB) => {
	...
	const removeSort = (id: string | string[]) => {
		setSorts((prev) => {
			if (Array.isArray(id)) {
	...
	const resetSorts = () => setSorts(initialSorts);

	const changeSorts = (sorts: SortState) => setSorts(sorts);
	...
	const toggleSort = (
		...
		setSorts((prev) => {
			...
			if (existingSort.disableSortRemoval || sortOptions.disableSortRemoval) {
	...
	const getNextSortingOrder = (id: string) => {
